...
install_s-ui() {
    ...
    last_version="s-ui_1.4.1"

    echo -e "Beginning the installation of s-ui ${last_version}..."
    wget -N --no-check-certificate -O /tmp/s-ui-linux-$(arch).tar.gz https://github.com/kiryusento2017/demo/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz
    if [[ $? -ne 0 ]]; then
